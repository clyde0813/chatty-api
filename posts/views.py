...
class QuestionGetAPIView(generics.GenericAPIView):
    ...
    def get(self, request, username):
        if Profile.objects.filter(username__username=username).exists():
            ...
            logger.info('Question Get Success Username : ', str(username), ' IP : ',
                        str(get_client_ip(request)))
            return paginator.get_paginated_response(serializer.data)
        else:
            logger.error('Question Get Failed Username : ', str(username), ' IP : ',
                         str(get_client_ip(request)))
            ...
            if get_client_ip(request) == Profile.objects.filter(
                ...
                question_object = serializer.save(author_ip=get_client_ip(request), refusal_status=False,
                                                  ...
                                                  chatroom_password=choice(AdjectiveList.objects.values_list('word'))[
                                                                        0] + ' ' +
                                                                    choice(NounList.objects.values_list('word'))[0])
                logger.info('Question Post Success Target : ',
                            str(serializer.validated_data['target_profile']), ' Content : ',
                            str(question_object.content), ' IP : ', str(get_client_ip(request)))
                return Response(
    ...
    def delete(self, request):
        if request.user.is_authenticated:
            ...
            if question_object.exists() is True:
                ...
                logger.error('Question Delete Failed - No Question Username : ', str(request.user.username), ' IP : ',
                             str(get_client_ip(request)))
    ...
    def post(self, request):
        if request.user.is_authenticated:
            ...
            if question_object.exists() is True:
                ...
                logger.error('Question Reject Failed - No Question Username : ', str(request.user.username), ' IP : ',
                             str(get_client_ip(request)))
        ...
        if request.user.is_authenticated:
            ...
            if question_instance.exists() is True:
                if question_instance.get().author_ip != get_client_ip(request):
                    ...
                    logger.error('Answer Post Failed - Bot Detection Username : ', str(request.user.username), ' IP : ',
                                 str(get_client_ip(request)))
